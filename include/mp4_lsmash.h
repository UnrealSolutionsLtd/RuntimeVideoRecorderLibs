...
#define MP4_LOG_ERROR( ... )                
#define MP4_LOG_WARNING( ... )              
#define MP4_LOG_INFO( ... )                 
#define MP4_FAIL_IF_ERR( cond, ... )  check(!cond)  
...
static void remove_mp4_hnd( mp4_hnd_t* p_mp4)
{
    if( !p_mp4 )
        return;
...
static int close_file( mp4_hnd_t* handle, int64_t largest_pts, int64_t second_largest_pts )
{
    mp4_hnd_t *p_mp4 = (mp4_hnd_t*)handle;
...
static int open_file( char *psz_filename, mp4_hnd_t* p_mp4)
{
    MP4_FAIL_IF_ERR(!p_mp4, "invalid handler for mp4 muxer information.\n");

    p_mp4->b_dts_compress = false;
    p_mp4->b_use_recovery = 0; // we don't really support recovery
    p_mp4->b_fragments    = false;
    p_mp4->b_stdout       = !strcmp( psz_filename, "-" );

    p_mp4->p_root = lsmash_create_root();
    MP4_FAIL_IF_ERR_EX( !p_mp4->p_root, "failed to create root.\n" );

    int openFileRes =lsmash_open_file(psz_filename, 0, &p_mp4->file_param);
    if (openFileRes < 0)
    {
        return openFileRes;
    }
    ...
    if (p_mp4->b_fragments)
    {
        int resFileMode = (int)p_mp4->file_param.mode | int(LSMASH_FILE_MODE_FRAGMENTED);

        p_mp4->file_param.mode = (lsmash_file_mode)resFileMode;
    }
...
static int set_param( mp4_hnd_t* handle, x264_param_t *p_param )
{
    ...
    check( i_media_timescale < UINT32_MAX);

    /* Select brands. */
    lsmash_brand_type brands[6]; // = {0};
    brands[0] = brands[1] = brands[2] = brands[3] = brands[4] = brands[5] = ISOM_BRAND_TYPE_3G2A;
    ...
    char* handler_name = (char*)"L - SMASH Video Media Handler";
    media_param.media_handler_name = handler_name;
    if( p_mp4->b_use_recovery )
    {
...
static int write_headers( mp4_hnd_t* handle, x264_nal_t *p_nal )
{
    mp4_hnd_t *p_mp4 = handle;
    ...
    /* SEI */
    p_mp4->p_sei_buffer = (uint8_t*)malloc( sei_size );
...
static int write_frame( mp4_hnd_t* p_mp4, uint8_t *p_nalu, int i_size, x264_picture_t *p_picture )
{
    uint64_t dts, cts;
    ...
    int appendRes = lsmash_append_sample(p_mp4->p_root, p_mp4->i_track, p_sample);
